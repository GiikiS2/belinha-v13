...
module.exports = {
    name: "unlock",
    desc: "desbloqueia um canal para os membros",
   categoria: 'moderação'}
...
const row2 = new MessageActionRow()
    ...
    if (message.channel.permissionsFor(message.channel.guild.roles.everyone).has("SEND_MESSAGES"))return message.reply(`**<:smiledog:872720085574373436> | ${message.author}, Este canal já está desbloqueado! Use ${'`'}${config.prefix}lock${'`'} para travar!**`)
  //if(message.channel.guild.roles.everyone, { SEND_MESSAGES: true })return message.reply(`**<:smiledog:872720085574373436> | ${message.author}, Este canal já está desbloqueado! Use ${'`'}${config.prefix}lock${'`'} para travar!**`)

    message.channel.permissionOverwrites.edit(message.channel.guild.roles.everyone, { SEND_MESSAGES: true });
    
    message.channel.send({content: `<a:checkmark69:846197278335172619> | **O Canal foi destrancado com sucesso!**`,
