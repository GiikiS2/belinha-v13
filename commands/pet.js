...
module.exports.run = async (client, message, args) => {
  ...
  if(people.id !== message.author.id) {
    ...
    botoes = [button1, button2]
  }

  const cat1 = new MessageActionRow().addComponents(botoes)
  ...
  message.reply({
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.customId === "ver") {
        if (data.pufflet == 'noinfo') return m.edit({
          embeds: [],
          content: `${people.username} não tem um pet, pode adotar um com b*loja`,
          components: [back]
        })
        if (people.id !== message.author.id) {
          if (data.pufflen == 'noname') return m.edit({
            embeds: [],
            content: `O pet de ${people.username} não tem um nome, ${people.username} pode dar um para ele, com o botão do painel`,
            components: [back]
          })
        }
        if (data.pufflen == 'noname') return m.edit({
      ...
      if (i.customId === "nome") {
        ...
        m.edit({
          components: [],
          embeds: [principembed.setDescription('digite abaixo o nome de seu pet.')]
        })
        const filter = m => m.author.id === message.author.id;
        const collectorm = message.channel.createMessageCollector({
          filter,
          time: 10 * 60000,
          max: 1
        });
